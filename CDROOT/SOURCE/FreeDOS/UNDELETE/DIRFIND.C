...
/* looks in the dir at cluster (0 for FAT1x root) for the dir */
/* named name and returns a new value for cluster... */

int
dirfind1 (char *name, Dword * cluster, struct DPB *dpb)
{
  Byte buffer[512];
  struct dirent dent;
  struct dirent *srch;
  ...
  Dword maxcluster;
  Dword myclust = cluster[0];
  maxcluster = (dpb->secperfat==0) ? dpb->FAT32_clusters : dpb->maxclustnum;

#ifdef DIRFINDDEBUG
  printf ("dirfind1(<%s>,<%lu>,<dpb>)\n", name, myclust);
#endif

  cluster[0] = 0xffffffffUL;		/* default is failure */
  if (myclust > maxcluster)
    {
  ...
  if (myclust == 0)			/* only used for FAT1x */
    {
      sector = dpb->numressec + (dpb->fats * dpb->secperfat); /* FAT1x only */
    }
  else
    {
	...
	{			/* scan one sector */
	  ...
	  if (srch->f_attrib & D_DIR)
	      ...
	      if (l == (8 + 3))
		{
		  ...
		  if (dpb->secperfat==0)	/* FAT32 */
		    {
		    cluster[0] = srch->cluster_hi;
		    cluster[0] <<= 16;
		    cluster[0] |= srch->cluster;
                    ...
                    if (srch->cluster_hi)
		      {
		        printf ("Warning: ignored cluster_hi on FAT1x: %u\n",
			      srch->cluster_hi);
                      }
                    }
#ifdef DIRFINDDEBUG
		  printf ("Directory <%s> is at %lu\n",
...
int
dirfind (char *name, Dword * cluster, struct DPB *dpb)
{
  Dword mycluster = 0;		/* FAT1x root dir is outside cluster space */
  char subname[8 + 1 + 3 + 1];	/* one part */
  int i;

  if (dpb->secperfat==0)
    mycluster = dpb->FAT32_rootdirstart; /* FAT32 root dir is in cluster space */
  cluster[0] = 0xffffffffUL;		/* default is failure */
...
#ifdef DIRFINDDEBUG
      printf ("Searching on: cluster %lu, directory ...\\%s\n",
	      mycluster, name);
#endif
    }

#ifdef DIRFINDDEBUG
  printf ("Full path points to cluster %lu\n", mycluster);
#endif
  cluster[0] = mycluster;
  return 0;
}
