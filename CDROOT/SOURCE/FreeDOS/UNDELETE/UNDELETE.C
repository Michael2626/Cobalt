...
main (int argc, char **argv)
{
  ...
  if ((dpb.bytepersec != 512) || (drive < 0))
    {
      printf ("\nERROR: Only 512 bytes per sector FAT disks supported\n");
      return -1;
    }
  ...
  if (cluster == 0)		/* FAT1x only */
    sector = dpb.numressec + (dpb.fats * dpb.secperfat); /* FAT1x */
  else
  ...
  if (!strcasecmp (argv[1], "/SYSSAVE"))
      ...
      if (!strcasecmp (argv[2], "ROOT"))
	{
	  if (dpb.secperfat == 0)	/* FAT32 */
	    {
	      printf("ERROR: Root directory is no special area on FAT32, please use DIRSAVE\n");
	      return -1;
	    } 
...
/* ***************************************************** */
  ...
  if ((mode == 1) || (!strcasecmp (argv[1], "/FOLLOW")))
	...
	{			/* EXTRACT mode */
	  ...
	  if (state < 0)
	    {
	      printf("ERROR: Failed to read FAT entry %lu\n", cluster);
	      return -1;
	    }
	  if (nextclust == 0)
	    {
	...
	{			/* FOLLOW mode */
	  ...
	  if (nextclust > 0)
	      ...
	      while (nextclust > 0)
		{
	      ...
	      printf ("Starting with cluster %lu\n", cluster);
	    }
	}
...
/* ***************************************************** */
  ...
  if (!strcasecmp (argv[1], "/DIRSAVE"))
	...
	{			/* path */
      ...
      printf ("DIRSAVE starting with sector %ld, cluster %lu\n",
	      sector, cluster);
      j = 0;			/* sector in cluster */
...
{
  printf
    ("FreeDOS Undelete - (C) Copyright 2001-2008 Eric Auer <eric@coli.uni.sb.de>\n");
  printf
    ("Undelete is free software; it is distributed under the terms of the GNU\n");
  printf
    ("General Public License. See COPYING for details. Only for FAT filesystems.\n\n");
