...
printdirents (Byte * buffer, struct DPB *dpb)
{
  ...
  for (i = 0; i < 512; i += 32)
      ...
      if (buffer[i] != 0)
	{
	  ...
	  if (DE->f_name[0] == D_DELCHAR)
	    {
	      if (dpb != (struct DPB *) 0)
		{
		  if ((dpb->secperfat!=0) && (cluster>0xffffUL))
		    {
		      printf ("FAT1x, ignored cluster_hi: %u\n", DE->cluster_hi);
		      cluster &= 0xffff;
		    }
		  state = readfat (dpb->drive, cluster, &fatchain, dpb);
	  ...
	  printf (" @%9.9lu, size %10.10lu", cluster, DE->f_size);
...
#if 0
//	  if (DE->cluster_hi)	/* should be zero unless LFN and/or FAT32? */
//	    {
//	      if ((DE->f_attrib & D_LONG) != D_LONG)
		      ...
		      if (response == 'Y' || response == 'y')
			{
			  ...
			  if (externalDirectory)
			      ...
			      do
				{
			      ...
			      writefat (dpb->drive, oldcluster, 0x0FFFFFFFUL, dpb);	/* EOF */
...
void
Follow (Dword cluster, const char *filename,
